...
def test_run_conversation_codex_falls_back_to_local_ollama_on_client_error(monkeypatch):
    created_clients = []
    ...
    class _FakeOpenAI:
        def __init__(self, **kwargs):
            ...
            self.chat = SimpleNamespace(
                completions=SimpleNamespace(create=MagicMock(return_value=_chat_message_response("Recovered via Ollama")))
            )
    ...
    class _CodexClientError(RuntimeError):
        def __init__(self):
            super().__init__("Error code: 400 - invalid model")
            self.status_code = 400

    monkeypatch.setattr(run_agent, "OpenAI", _FakeOpenAI)
    monkeypatch.setenv("HERMES_CODEX_FALLBACK_MODEL", "qwen3.5:35b")
    monkeypatch.setenv("HERMES_CODEX_FALLBACK_BASE_URL", "http://127.0.0.1:11434")

    agent = _build_agent(monkeypatch)
    monkeypatch.setattr(agent, "_run_codex_stream", lambda api_kwargs: (_ for _ in ()).throw(_CodexClientError()))

    result = agent.run_conversation("reply with a local fallback")

    assert result["completed"] is True
    assert result["final_response"] == "Recovered via Ollama"
    assert agent.provider == "ollama-local"
    assert agent.api_mode == "chat_completions"
    assert agent.model == "qwen3.5:35b"
    assert created_clients[-1].kwargs["base_url"] == "http://127.0.0.1:11434/v1"
    assert created_clients[-1].kwargs["api_key"] == "ollama"
