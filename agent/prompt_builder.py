...
def build_skills_system_prompt() -> str:
    ...
    for category in sorted(skills_by_category.keys()):
        ...
        for name, desc in sorted(skills_by_category[category], key=lambda x: x[0]):
            ...
            seen.add(name)
            if desc:
                index_lines.append(f'    - name="{name}" description="{desc}"')
            else:
    ...
    return (
        ...
        "When calling skill_view(name), pass only the exact skill name from name=\"...\" "
        "and never the description text. "
    ...
    if soul_path:
        try:
            ...
            if content:
                ...
                sections.insert(0,
                    f"## SOUL.md (PRIMARY IDENTITY — THIS OVERRIDES DEFAULT AGENT IDENTITY)\n\n"
                    f"You ARE the persona described in SOUL.md. This is not a suggestion — "
                    f"it is your identity. Embody its voice, tone, and personality in every "
                    f"reply. Never fall back to generic assistant behavior when SOUL.md is present.\n\n{content}"
                )
        except Exception as e:
