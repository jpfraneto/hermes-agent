...
def _candidate_skill_names(raw_name: str) -> List[str]:
    """
    Generate progressively more-normalized skill name candidates.

    Models sometimes copy a skills index line like:
      "anky: Write one real daily ..."
    and pass the whole string to skill_view(name=...).
    Accept the exact name first, then fall back to common trimmed variants.
    """
    if not raw_name:
        return []
    ...
    for separator in (":", " - ", " -- ", " -", " | "):
        if separator in trimmed:
            add(trimmed.split(separator, 1)[0])
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        for candidate_name in _candidate_skill_names(name):
            ...
            for found_md in SKILLS_DIR.rglob(f"{candidate_name}.md"):
