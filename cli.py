...
from agent.skill_commands import (
    ...
    build_auto_skill_invocation_message,
)
...
class HermesCLI:
    ...
    def __init__(
        ...
        self._session_model_override = model is not None
    ...
    def _refresh_model_from_disk(self) -> None:
        """Reload the default model from ~/.hermes/.env/config.yaml for fresh sessions."""
        if self._session_model_override:
            return

        refreshed_model: Optional[str] = None

        if _user_env.exists():
            try:
                for raw_line in _user_env.read_text().splitlines():
                    line = raw_line.strip()
                    ...
                    if key in {"LLM_MODEL", "OPENAI_MODEL"} and value:
                        refreshed_model = value
                        break
            except Exception:
                pass
            ...
            if config_path.exists():
                try:
                    ...
                    if isinstance(model_cfg, str) and model_cfg.strip():
                        refreshed_model = model_cfg.strip()
                    elif isinstance(model_cfg, dict):
                        default_model = model_cfg.get("default")
                        if isinstance(default_model, str) and default_model.strip():
                            refreshed_model = default_model.strip()
                except Exception:
                    pass
    ...
    def reset_conversation(self):
        ...
        if self.agent and self.conversation_history:
            try:
                ...
                pass
        self._refresh_model_from_disk()
        # Force a fresh client on the next turn so runtime/config changes apply.
        self.agent = None
    ...
    def process_command(self, command: str) -> bool:
        ...
        if cmd_lower in ("/quit", "/exit", "/q"):
            ...
            if len(parts) > 1:
                ...
                self._session_model_override = True
                self.agent = None  # Force re-init
                # Save to config
    ...
    def chat(self, message: str) -> Optional[str]:
        ...
        auto_skill = build_auto_skill_invocation_message(message)
        if auto_skill:
            effective_message = auto_skill["message"]
            print(f"\n⚡ Auto-loading skill: {auto_skill['skill_name']}")
